...
$(document).ready(function(){
        ...
        $.getJSON('obteneridinsumo',{insumo:$('#insumo').val()},function(data){
            ...
            else
            {
                fila = "<tr style='background-color: lightgreen' id="+data+"><td class='vacia'></td>" +
                    ...
                    "<td ><input class='form-control input cantidad' type='text'  value="+$('#cantidad').val()+"></td>" +
                    "<td><input type='text' class='form-control precio'></td><td  class='accion text-center'>" +
    ...
    $('.btn_guardaredicion').click(function(){
        ...
        $.getJSON('actualizarrequisicion',{precios:precios,cantidades:cantidades,insumos:codigosinsumos,id:this.id},function(data){
            if(data===true)
            {
                new PNotify({
                    title:'muy bien',
